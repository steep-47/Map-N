...
function parseHeaderLocation(text){const lines=String(text||'').split(/\n/).slice(0,16).map(cleanMetaLine).filter(Boolean);for(const line of lines){const p=labeledLocation(line);if(p?.length)return p;}for(const line of lines){const m=line.match(/[【[]\s*([^】\]\n]+)\s*[】\]]/u);if(m&&(TIME_PREFIX_RE.test(m[1])||BARE_TIME_PREFIX_RE.test(m[1]))){const p=normalizeLocationParts(m[1]);if(p.length)return p;}}for(let n=0;n<lines.length;n++){const line=lines[n];if(TIME_PREFIX_RE.test(line)||BARE_TIME_PREFIX_RE.test(line)){const p=normalizeLocationParts(line);if(p.length)return p;}if(timeMeta(line)){for(let k=n+1;k<=Math.min(n+2,lines.length-1);k++){const candidate=stripOuterBrackets(lines[k]).replace(/^[📍🧭]\s*/u,'').trim(),p=normalizeLocationParts(candidate);if(locationShape(p,candidate))return p;}}}return null;}
