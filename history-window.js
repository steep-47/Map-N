...
async function install(){for(let i=0;i<160&&!window.MapNInstance;i++)await wait(25);const inst=window.MapNInstance;if(!inst||inst.__historyWindow102)return;inst.__historyWindow102=true;inst.historyScanLimit=limit();inst.scanChat=function(){const fresh=window.SillyTavern?.getContext?.();if(fresh)this.ctx=fresh;const chat=this.ctx?.chat||[],r=range(chat);batch(this,()=>{for(let i=r.start;i<r.end;i++){const m=chat[i];if(m?.mes)this.process(String(m.mes),!!m.is_user);}});this.__mapNLastHistoryScan=r;return r;};console.log(`[Map-N] history window v1.0.2 installed: recent ${limit()} floors`);}
install();
