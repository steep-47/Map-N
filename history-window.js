...
async function install(){
  ...
  const inst=window.MapNInstance;if(!inst||inst.__historyWindow103)return;inst.__historyWindow103=true;inst.historyScanLimit=limit();
  // Base Map-N calls save() for every processed message. During a bounded rebuild this used to serialize
  // the whole accumulated memory up to 200 times. Keep mutations in memory and persist once at batch end.
  ...
  console.log(`[Map-N] history window v1.0.3 installed: recent ${limit()} floors`);
}
install();
