...
async function waitForMapN(timeout = 5000) {
    const start = Date.now();
    while (Date.now() - start < timeout) {
        if (window.MapNInstance?.toggle) return window.MapNInstance;
...
function installWandEntry() {
    ...
    const openMap = async event => {
        ...
        const inst = await waitForMapN();
        if (!inst) {
            console.error('[Map-N] 菜单入口点击后实例未就绪');
            return;
        }
        inst.toggle();
        menu.style.display = 'none';
    };
    entry.addEventListener('click', openMap);
...
try {
    ...
    await import('./Map-N.js?v=1.2.3');
    await import('./scene-scanner.js?v=1.2.3');
