// Map-N presence resolver v2.0.1
...
async function install(){for(let n=0;n<200&&(!window.MapNInstance||!globalThis.MapNEntityCore);n++)await wait(50);const i=window.MapNInstance,core=globalThis.MapNEntityCore;if(!i||!core||i.__presence201)return;for(let n=0;n<80&&!i.__strictEntity201;n++)await wait(50);i.__presence201=true;i.__mapNPresenceState=load(i);const old=i.process.bind(i);i.process=function(text,isUser=false){const before=[...(this.currentChars||[])];old(text,isUser);if(isUser||!text){if(isUser)this.currentChars=before;return;}this.currentChars=resolve(this,core,String(text),this.currentChars||[]);for(const x of this.currentChars)this.encountered.add(x);this.save?.();if(this.container?.classList.contains('open'))this.render?.();};const chat=i.ctx?.chat||[],r=recentRange(chat);for(let n=r.end-1;n>=r.start;n--){const m=chat[n];if(m?.mes&&!m.is_user){i.currentChars=resolve(i,core,String(m.mes),i.currentChars||[]);break;}}i.save?.();i.render?.();console.log('[Map-N] presence resolver v2.0.1 installed');}
install();
