...
const wait=ms=>new Promise(r=>setTimeout(r,ms)),ROOT='世界舆图',MEMORY_VERSION=8;
...
function mergeRecord(i,r){if(!r)return;const aliases=uniq([...(r.aliases||[]),r.label]),existing=i.nodeMap[r.id],sceneContent=['剧情场景头确认的地点。',...(r.evidence||[]).map(e=>e.text)].join(' ');if(!existing){i.nodeMap[r.id]={id:r.id,displayName:r.label,aliases:[...aliases],content:sceneContent,type:'location',children:[],parent:r.parent,isWater:/海|河|湖|江|溪|潭|湾|岸|滩/u.test(r.label),isMountain:/山|峰|岭|崖|峪|谷/u.test(r.label),learned:true,source:'scene-header'};}const n=i.nodeMap[r.id];const sceneOwned=n.source==='scene-header'&&n.learned===true;if(sceneOwned){n.displayName=r.label;n.parent=r.parent;n.content=sceneContent;n.learned=true;n.source='scene-header';}n.aliases=uniq([...(n.aliases||[]),...aliases]);for(const a of n.aliases)if(!i.alias.has(a))i.alias.set(a,r.id);i.discovered.add(r.id);if(sceneOwned){if(r.parent===ROOT){i.root.children||=[];if(!i.root.children.includes(r.id))i.root.children.push(r.id);}else if(i.nodeMap[r.parent]){i.nodeMap[r.parent].children||=[];if(!i.nodeMap[r.parent].children.includes(r.id))i.nodeMap[r.parent].children.push(r.id);}}}
function mergeAll(i,core){migrate(i,core);for(const r of Object.values(i.__mapNSceneStore?.learnedLocations||{}))mergeRecord(i,r);}
function recordConflict(i,incoming,kept,src,text){
 const s=i.__mapNSceneStore||=load(i),item={incoming,kept,source:src||null,evidence:evidenceText(text),at:Date.now()};
 const last=s.conflicts?.at?.(-1);if(!last||last.incoming!==incoming||last.kept!==kept)s.conflicts=[...(s.conflicts||[]),item].slice(-24);
 save(i);
}
function stableParts(i,core,clean,src,text){
 if(!clean.length)return clean;const s=i.__mapNSceneStore||=load(i),incoming=clean.join('／'),leaf=clean.at(-1);
 const target=core.normalizeLocationSegment(leaf),matches=Object.values(s.learnedLocations||{}).filter(r=>core.normalizeLocationSegment(r?.label)===target);
 if(matches.length!==1)return clean;const rec=matches[0],old=String(rec.id||'').split('／').filter(Boolean);if(!old.length||rec.id===incoming)return clean;
 if(clean.length===1)return old;
 if(old.length>=2&&clean.length>=2&&old[0]===clean[0]){recordConflict(i,incoming,rec.id,src,text);return old;}
 return clean;
}
...
async function install(){for(let n=0;n<160&&(!window.MapNInstance||!globalThis.MapNEntityCore);n++)await wait(50);const i=window.MapNInstance,core=globalThis.MapNEntityCore;if(!i||!core||i.__sceneScanner220)return;i.__sceneScanner220=true;i.__mapNSceneStore=load(i);migrate(i,core);const priorFlush=i.__mapNFlushBatch?.bind(i);i.__mapNFlushBatch=function(){priorFlush?.();if(this.__mapNSceneSaveDirty){this.__mapNSceneSaveDirty=false;write(this);}};const oldBuild=i.build.bind(i);i.build=function(entries){oldBuild(entries);mergeAll(this,core);};const oldProcess=i.process.bind(i);i.process=function(text,isUser=false){oldProcess(text,isUser);if(!isUser&&text){const paths=core.parseHeaderLocations?.(text)||[];for(const parts of paths)learn(this,core,parts,true,null,String(text));}if(this.container?.classList.contains('open'))this.render?.();};mergeAll(i,core);const chat=i.ctx?.chat||[],r=recentRange(chat);for(let n=r.start;n<r.end;n++){const m=chat[n];if(m?.mes&&!m.is_user){const text=String(m.mes),paths=core.parseHeaderLocations?.(text)||[];for(const p of paths)learn(i,core,p,true,n,text);}}globalThis.MapNSceneScanner={reconcileRecent:(inst,start,end,all)=>reconcileRecent(inst,start,end,all)};const es=i.ctx?.eventSource,et=i.ctx?.eventTypes||i.ctx?.event_types;if(es&&et){const onMutation=()=>setTimeout(()=>{const fresh=window.SillyTavern?.getContext?.();if(fresh)i.ctx=fresh;const all=i.ctx?.chat||[],rr=recentRange(all);if(reconcileRecent(i,rr.start,rr.end,all))i.render?.();},80);['MESSAGE_SWIPED','MESSAGE_UPDATED','MESSAGE_DELETED'].forEach(k=>{if(et[k])es.on(et[k],onMutation)});}i.render?.();console.log('[Map-N] scene scanner v2.3.0 installed');}
install();
