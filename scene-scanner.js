...
const ROOT_PLACEHOLDER_UPGRADE_RE=/(?:大陆|陆|洲|州|郡|府|县|国|皇朝|王朝|帝国|王国|域|界)$/u;
function adoptStaticPlaceholder(i,r){
 ...
 const dynamicParentLabel=r.parent===ROOT?ROOT:String(r.parent||'').split('／').at(-1),oldParent=old.parent||ROOT;
 if(oldParent===ROOT&&!ROOT_PLACEHOLDER_UPGRADE_RE.test(r.label))return null;
 if(oldParent!==ROOT&&oldParent!==r.parent&&oldParent!==dynamicParentLabel)return null;
...
function learn(i,core,parts,setCurrent=true,sourceIndex=null,sourceText=''){
 ...
 if(resolved.conflicted||!resolved.parts?.length){if(setCurrent){i.currentPos=null;i.path=[ROOT];}i.save?.();return null;}const clean=resolved.parts;
