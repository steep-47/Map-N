// Map-N worldbook geography parser v2.2.0
...
async function install(){for(let n=0;n<180&&!window.MapNInstance;n++)await wait(50);const i=window.MapNInstance;if(!i||i.__worldbookGeoParser210)return;i.__worldbookGeoParser210=true;const original=i.build.bind(i);i.build=function(entries){const raw=Array.isArray(entries)?entries:[],{geo,parents}=compile(raw);this.__mapNRawWorldEntries=raw;this.__mapNGeoEntries=geo;original(geo);applyCompiledParents(this,parents)};if(Array.isArray(i.entries)&&i.entries.length){const raw=i.__mapNRawWorldEntries||[...i.entries];i.build(raw);i.save?.();i.render?.()}console.log('[Map-N] worldbook geography parser v2.2.0 installed')}
install();
